...
dt_styles_get_item_list (const char *name, gboolean params, int imgid)
{
  ...
  if ((id=dt_styles_get_id_by_name(name)) != 0)
  {
    ...
    else if (imgid != -1)
    {
      ...
      DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db), "select num, operation, multi_name, enabled, (select num from history where imgid=?2 and operation=style_items.operation and multi_priority=style_items.multi_priority order by num desc limit 1) from style_items where styleid=?1 UNION select -1,history.operation,history.multi_name,history.enabled,history.num from history where imgid=?2 and history.enabled=1 and history.operation not in (select operation from style_items where styleid=?1) group by operation having max(num) order by num desc", -1, &stmt, NULL);
      DT_DEBUG_SQLITE3_BIND_INT(stmt, 2, imgid);
    }
    else
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
      ...
      {
        g_snprintf(name,512,"%s %s (%s)",dt_iop_get_localized_name((gchar *)sqlite3_column_text (stmt, 1)),sqlite3_column_text (stmt, 2), (sqlite3_column_int (stmt, 3)!=0)?_("on"):_("off"));
        item->params = NULL;
